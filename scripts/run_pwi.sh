...
# ============================================================
# ADS PWI Pipeline Runner
# ============================================================

# Default subject path (relative to OpenADS folder)
DEFAULT_SUBJECT_PATH="assets/examples/pwi/sub-02e8eb42"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --stages)
            ...
            STAGES_LIST="$2"
            shift 2
            # Handle case where user put spaces after commas
        ...
        --help|-h)
            ...
            echo "                        Example: --config configs/custom_pwi.yaml"
            echo ""
            ...
            echo "                        Available stages: prepdata, gen_mask, skull_strip, gen_ttp,"
            echo "                                         registration, ttpadc_coreg, inference, report"
            echo ""
            ...
            echo "  $0 assets/examples/pwi/sub-12345"
            echo ""
            echo "  # Run all stages (override YAML)"
            echo "  $0 assets/examples/pwi/sub-02e8eb42 --all"
            echo ""
            echo "  # Run specific stages (no spaces)"
            echo "  $0 assets/examples/pwi/sub-02e8eb42 --stages prepdata,gen_mask"
            echo ""
            echo "  # Run specific stages (with spaces - use quotes)"
            echo "  $0 assets/examples/pwi/sub-02e8eb42 --stages \"prepdata, gen_mask, inference\""
            echo ""
            echo "  # Use specific GPU"
            echo "  $0 assets/examples/pwi/sub-02e8eb42 --gpu 0 --all"
            echo ""
            echo "  # Complete example"
            echo "  $0 assets/examples/pwi/sub-02e8eb42 \\"
            echo "     --config configs/pwi_pipeline.yaml \\"
            echo "     --gpu 1 \\"
            echo "     --stages prepdata,gen_mask,skull_strip,gen_ttp,registration,ttpadc_coreg,inference,report"
            echo ""
            exit 0
            ;;
        -*)
